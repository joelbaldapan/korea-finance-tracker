...
export class IngestController {
    ...
    constructor(parserFactory, fxRateService, merchantEnricher, geocodingService, transactionRepository) {
    ...
    async handleIngest(c) {
        try {
            ...
            let enriched = { cleanStoreName: null, hangulName: null, category: 'Enrichment Failed', isOnline: false };
            try {
            ...
            if (!enriched.isOnline) {
                ...
                geocode = await this.geocodingService.geocode(keyword, device_location?.lat, device_location?.lng);
            }
    ...
    async handleUserImport(c, userId) {
        try {
            const body = await c.req.json();
            ...
            for (const item of items) {
                ...
                try {
                    ...
                    const fx = await this.fxRateService.getRates(parsed.transactedAt, parsed.originalCurrency);
                    let enriched = { cleanStoreName: null, hangulName: null, category: skip_enrichment ? 'Uncategorized' : 'Enrichment Failed', isOnline: false };
                    if (!skip_enrichment) {
                        try {
                            enriched = await this.merchantEnricher.enrich(parsed.rawMerchant);
                        }
    ...
    async handleUpdate(c, userId, transactionId) {
        try {
            const body = await c.req.json();
    ...
    async handleManualEntry(c, userId) {
        try {
            const body = await c.req.json();
